...
t2 = []
(0...5*t.size).each do |y|
  t2[y] = []
  (0...5*t.first.size).each do |x|
    y0, x0 = y % t.size, x % t[y % t.size].size
    dy, dx = (y-y0)/t.size, (x-x0)/t[y % t.size].size
    t2[y][x] = t[y0][x0] + dx + dy
    t2[y][x] -= 9 if t2[y][x] >= 10
  end
end

puts "part2", explore(t2)
